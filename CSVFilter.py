...
class Application(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.iconbitmap('Datasource/logo.ico')
...
class DataTable(ttk.Treeview):
    ...
    def find_value(self, pairs):
        ...
        new_df = self.stored_dataframe.astype(str)
        for col, value in pairs.items():
            query_string = f"{col}.str.contains('{value}',na=False,case=False)"
...
class SearchPage(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent)
        self.Frame1 = tk.Frame(parent, background="blue")
        self.Frame1.place(relheight=1, relwidth=0.25)
        self.file_names_listbox = tk.Listbox(self.Frame1, selectmode=tk.SINGLE, background="lightgreen")
        self.file_names_listbox.place(relheight=.99, relwidth=.99)
